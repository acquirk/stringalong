...
$result = mysql_query("SELECT * FROM quirktest ORDER BY urltime DESC LIMIT 0,1")
or die(mysql_error());  
...
echo "<h2>Stringalong1</h2>";

// Retrieve all the data from the "quirktest" table
$result = mysql_query("SELECT url, urltime, urldate, title FROM quirktest ORDER BY urldate DESC")
or die(mysql_error());  
...
$smalltitle = substr($title, 0, 22); 
	...
	// Print out the contents of each row into a table
	echo "<ol>";
	echo "<li><a href='$url' target='iframe' title='$title'>$smalltitle</a></li>";
	echo "</ol>";
} 
?>
...
<iframe height="100%" width="100%" name="iframe" src="http://stringalong.bullemhead.com/start.php" scrolling="yes" title="iframe"></iframe>
